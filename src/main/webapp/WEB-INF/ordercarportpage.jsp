        ...
        <table>
            <tr>
                <td>    
                    ...
                    <rect x="50" y="<%= draw.sideRemnant(order, "roof") + 50%>" height="<%= order.getLength()%>" width="<%= order.getWidth()%>"
                          style="stroke:#000000; stroke-width: 4; fill: #F5F5DC"/>
                    
                    <%= draw.carport(order, "roof")%>
                    
                    <line x1="50" y1="50" x2="<%= order.getWidth() + 50%>" y2="<%=order.getLength() + 50%>" style="stroke:rgb(255,0,0);stroke-width:2" />
                    
                    <line x1="50" y1="<%=order.getLength() + 50%>" x2="<%=order.getWidth() + 50%>" y2="50" style="stroke:rgb(255,0,0);stroke-width:2" />
                    
                    <line x1="25" y1="50" x2="25" y2="<%=order.getLength() + 50%>" style="stroke:rgb(255,0,0);stroke-width:2" />
                    <text x="25" y="<%=order.getLength()/2 + 50%>"
                          font-family= "Verdana" font-size="12px"
                          text-anchor= "middle" alignment-baseline=\"middle\">
                    <%= order.getLength() + " cm"%>
                    </text>
                    
                    <line x1="<%= order.getWidth() + 50%>" y1="<%= order.getLength() + 70%>" x2="50" y2="<%= order.getLength() + 70%>" style="stroke:rgb(255,0,0);stroke-width:2" />
                    
                    <text x="<%=(order.getWidth())/2 + 50%>" y="<%=order.getLength() + 85%>"
                          font-family= "Verdana" font-size="12px"
                          text-anchor= "middle" alignment-baseline=\"middle\">
                    <%= order.getWidth() + " cm"%>
                    </text>
